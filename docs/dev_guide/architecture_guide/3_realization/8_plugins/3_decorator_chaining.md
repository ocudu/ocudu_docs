...
create_phy_tap_factory()
  └─ epre_zmq_factory::create()
       └─ epre_zmq_decorator
            └─ my_ul_processor_factory::create()
                 └─ my_ul_processor   ← innermost, runs first
```
...
```cpp
class epre_zmq_decorator : public external_ul_processor
{
public:
  epre_zmq_decorator(std::unique_ptr<external_ul_processor> inner,
...
```cpp
class epre_zmq_factory : public external_ul_processor_factory
{
public:
  epre_zmq_factory(std::shared_ptr<external_ul_processor_factory> inner_factory,
  ...
  {
    return std::make_unique<epre_zmq_decorator>(
...
The entry point builds the chain from the argument string. Adding the ZMQ decorator requires no changes to `my_ul_processor` or its factory - it is purely additive:
                       ...
                       const std::string& args)
{
  ...
  std::shared_ptr<external_ul_processor_factory> factory =
      std::make_shared<my_ul_processor_factory>(nof_rb, nof_ports);

  // If tap_ul_epre=<address> appears in args, wrap with the ZMQ decorator.
  std::smatch m;
  if (std::regex_search(args, m, std::regex(R"(tap_ul_epre=([^,]*))"))) {
    factory = std::make_shared<epre_zmq_factory>(std::move(factory), m[1].str());
  }
