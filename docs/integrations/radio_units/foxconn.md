...
This guide provides further details on connecting the OCUDU CU/DU to an RU using the the O-RAN 7.2 split. Specifically, we discus the Foxconn RPQN family of devices.
...
The full configuration example file we used for this set up can be found [`here`](assets/foxconn_rrh_config.txt). In this config we are configuring the RU for 20 MHz.
