...
A [Leo Bodnar GPS reference clock](http://www.leobodnar.com/shop/index.php?main_page=product_info&cPath=107&products_id=234) was used as the USRP clock source for this set-up.
...
- [gnb_rf_b200_tdd_n78_20mhz.yml](https://gitlab.com/ocudu/ocudu/-/blob/dev/configs/gnb_rf_b200_tdd_n78_20mhz.yml?ref_type=heads)
...
The gNB is configured using the base configuration example found [here](https://gitlab.com/ocudu/ocudu/-/blob/dev/configs/gnb_rf_b200_tdd_n78_20mhz.yml?ref_type=heads), with some slight modifications.
...
:::note
OCUDU supports all NR bands. However, not all BWs are supported in all bands, to confirm that your configuration is correct, you should
reference *Table 5.3.5-1* in [TS 38.104](https://www.etsi.org/deliver/etsi_ts/138100_138199/138104/16.06.00_60/ts_138104v160600p.pdf).
:::
...
Download [pySim](https://github.com/osmocom/pysim):
...
More information on pySim and SUCI configuration can be found in [this guide](https://downloads.osmocom.org/docs/pysim/master/html/suci-tutorial.html) in the pySim documentation.
...
The Open5GS [5G Core Quickstart Guide](https://open5gs.org/open5gs/docs/guide/01-quickstart/#:~:text=restart%20open5gs%2Dsgwud-,Setup%20a%205G%20Core,-You%20will%20need) provides a comprehensive overview of how to configure Open5GS to run as a 5G Core.
...
You can see how to register subscriber information with the core [here](https://open5gs.org/open5gs/docs/guide/01-quickstart/#:~:text=Register%20Subscriber%20Information).
...
![sim1](assets/sim_1.jpg) ![sim2](assets/sim_2.jpg)

If you cannot see the `5G` option in `Preferred network type`, then you may need to activate it. This can be enabled under the Developer Options, if you do not have access to Developer Options see [this guide](https://developer.android.com/studio/debug/dev-options).
...
![apn1](assets/apn_1.jpg) ![apn2](assets/apn_2.jpg)
...
![image](assets/force_nr.jpg)

When you select this option, you may see the `Preferred Network Type` field in the SIM configuration menu change to `4G/3G/2G (Auto)` as seen in the screenshot in the [Connecting to the Network section](#connecting-to-the-network).
...
- You should check that the gNB is transmitting correctly. This can be done with a spectrum analyzer or tools like [gr-fosphor](https://kb.ettus.com/Fosphor) and [Maia SDR](https://maia-sdr.org/). An example of a “healthy” gNB broadcast from Maia SDR can be seen here ![maia_screenshot](assets/maia_sdr.png)
...
![image](assets/nr_timer_wait_ims_registration.png)![image](assets/support_ims_nr_registration_timer.png)
