...
- [Getting started with Helm](https://helm.sh/docs/intro/)
...
The CU/DU is provided by OCUDU. The Open Fronthaul (OFH) Library offers
...
supported O-RUs, see the [Radio Units](../../integrations/radio_units/index.md) overview under integrations.
...
For more information, refer to the [official K3s documentation](https://k3s.io/).
...
Register on the [Canonical website](https://ubuntu.com/pro) and create an account. After that, use your
...
[Ubuntu documentation](https://documentation.ubuntu.com/pro-client/en/docs/howtoguides/enable_realtime_kernel/).
...
using the OCUDU linuxptp Helm chart. As a first step, install the OCUDU Helm repository:
...
[official linuxptp documentation](https://linuxptp.nwtime.org/documentation/).
...
For additional configuration options, refer to the [linuxptp Helm chart README](https://gitlab.com/ocudu/ocudu_elements/ocudu_helm/-/tree/main/charts/linuxptp?ref_type=heads/).
An example values.yaml can be found in the repo as well.
...
necessary information to download and set up Open5GS for use with OCUDU:
...
- [Example PV and PVC for Open5GS](https://gitlab.com/ocudu/ocudu_elements/ocudu_helm/-/tree/main/charts/open5gs?ref_type=heads)
...
- [Example values.yaml for Open5GS](https://gitlab.com/ocudu/ocudu_elements/ocudu_helm/-/tree/main/charts/open5gs?ref_type=heads)
...
To deploy the gNB, edit the values.yaml file and set the desired RAN parameters. An example values.yaml for the OCUDU
Helm Chart can be found [here](https://gitlab.com/ocudu/ocudu_elements/ocudu_helm/-/tree/main/charts/ocudu-gnb).
...
The OCUDU RU Emulator is a tool that emulates a Radio Unit (RU). It prints KPIs such as early and late packets,
...
on your system, you can deploy cyclictest using the OCUDU rt-test Helm chart.
...
stress-ng, refer to the [cyclictest](https://wiki.linuxfoundation.org/realtime/documentation/howto/tools/cyclictest/start)
and [stress-ng](https://wiki.ubuntu.com/Kernel/Reference/stress-ng) documentation.
...
OCUDU Helm repository. The metrics server collects and parses gNB metrics, stores them in an InfluxDB
...
We use Telegraf to collect metrics from the running OCUDU instance.
Make sure that the `WS_URL` points to the right WebSocket URL and port that is configured in the OCUDU Helm.
