...
for proof-of-concept and initial testing to allow users to test OCUDU without the need for expensive hardware. It is not intended for scenarios which require a deployment ready solution.
:::
...
If you have not already done so, install the latest version of srsRAN 4G and all of its dependencies. This is outlined in the [installation guide](https://docs.srsran.com/projects/4g/en/latest/general/source/1_installation.html).

Please check our srsRAN 4G [ZeroMQ Application Note](https://docs.srsran.com/projects/4g/en/latest/app_notes/source/zeromq/source/index.html) for information on installing ZMQ and using it with srsRAN 4G.
...
![image](assets/gNB_srsUE_usrp.png)
...
**Note:** All routes have to be configured as presented in  [Routing Configuration for USRP-based setup](#ota-routing) section.
...
To read more about the UE console trace metrics, see the [UE User Manual](https://docs.srsran.com/projects/4g/en/latest/usermanuals/source/srsue/source/6_ue_commandref.html#ue-commandref).
...
![image](assets/gNB_srsUE_zmq.png)
...
**Note:** All routes have to be configured as presented in [Routing Configuration for ZMQ-based setup](#zmq-routing) section.
...
:::info
The Multi-UE scenario outlined here is **not** intended to be an optimized, performant, or scalable solution. It is a simple example to demonstrate how to connect multiple UEs to a single gNB
...
![image](assets/gr_flow_graph.png)
...
* [GNU-Radio flow graph](assets/multi_ue_scenario.grc)
...
* [subscriber_db.csv](assets/subscriber_db.csv)
...
Alternatively, you can download already edited `open5gs.env` file [here](assets/open5gs.env).
...
* ports for the ZMQ connections, that match the those listed in [Ports Used in the GNU-Radio flow graph](#grc-ports-table).
...
* [UE1 config](assets/ue1_zmq.conf)
* [UE2 config](assets/ue2_zmq.conf)
* [UE3 config](assets/ue3_zmq.conf)
...
![image](assets/gr_control_slider.png)
...
Once the setup is ready, you can start data traffic by following [Testing the Network](#zmq-testing) section for the ZMQ-based setup. Note that here we have three UEs (hence, three network namespaces, namely ue1,ue2,ue3), and the default route has to be configured for each of them.
...
To solve this routing issue, you can disable (or even remove) the manually installed Open5GS – please check sections 6 and/or 7 of the [Open5GS tutorial](https://open5gs.org/open5gs/docs/guide/01-quickstart/).
